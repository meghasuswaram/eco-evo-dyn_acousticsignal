...
% S(l) = stepinfo(nz);
S1(l) = sum(nzt(:,tmax)); %N*
% S2(l) = S.PeakTime; %zbar*
S2(l) = sum(ymutime(:,tmax)); %zbar*
pct = ((numel(S2(S2 < rs)))/5)*100;
end
% rminout(j,i) = S1;%N*
rminout2(j,i) = pct;%zbar*
end
end
...
% contourf(rmini, rmaxi, rminout, 'ShowText','on')
% colorbar
% xlabel({'rmin';'Decrease in reproductive incentive (as rmin gets closer to rmax) ==>'})
% ylabel({'rmax';'Increase in reproductive incentive ==>'})
% title({'Population Steady State';'N*'})
% xticks([200])
% yticks([1.4 2.3])
...
figure(8)
h = heatmap(rmini-dmin, rmaxi-dmax, rminout2)
h.YDisplayData = flipud(h.YDisplayData);  % equivalent to 'YDir', 'Reverse'
colorbar
xlabel({'rmin-dmin';'Increase in reproductive reward with low z -->'})
ylabel({'rmax-dmax';'Increase in reproductive incentive ==>'})
title('%Bimodality')
